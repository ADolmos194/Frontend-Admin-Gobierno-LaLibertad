...
import { Distritos } from '@/apis_modelos/general/distrito_servcie/distrito.model';
...
@Component({
    ...
    providers: [MessageService, DistritoService, ConfirmationService]
})
export class Distrito implements OnInit {
    distritoDialogo: boolean = false;
    distritos = signal<Distritos[]>([]);
    distrito: Distritos = {
        id: 0,
        nombre: '',
        provincia_id: 0,
    ...
    seleccionarDistritos!: Distritos[] | null;
    ...
    async cargarDistritos() {
        this.isLoading = true;
        try {
            const response: Distritos[] = await this.distritoService.getDistritos();
    ...
    async guardarDistrito() {
        ...
        try {
            ...
            const response = this.accion === 1
                ? await this.distritoService.createDistrito(DistritoParaEnviar)
                : await this.distritoService.updateDistrito(this.distrito.id, DistritoParaEnviar);
    ...
    editarDistrito(distrito: Distritos) {
        this.distrito = { ...distrito };
        this.accion = 2;
        this.distritoDialogo = true;
    }

    async eliminarDistrito(distrito: Distritos) {
        this.confirmationService.confirm({
            ...
            accept: async () => {
                this.isLoading = true;
                try {
                    const response = await this.provinciaService.deleteProvincia(distrito.id);
            ...
            reject: () => {
                this.messageService.add({ severity: 'info', summary: 'Cancelado', detail: 'No se eliminó el distrito' });
            }
        });
    }
}
